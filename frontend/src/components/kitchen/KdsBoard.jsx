...
import { Clock, ChefHat, CheckCircle, AlertCircle, Banknote, CreditCard, ArrowRight, PlusCircle } from 'lucide-react';
import { soundAlert } from '../../services/soundAlert';

export default function KdsBoard({ orders, onUpdateOrderStatus, onOpenProductManager }) {
      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white flex items-center gap-2">
                KDS Cocina <span className="text-xs bg-slate-800 text-amber-400 font-bold px-2 py-0.5 rounded-full border border-amber-400/30">Tablero Trello</span>
              </h1>
              <p className="text-xs text-slate-400 font-medium">Gestión de comandas y menú en tiempo real</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial px-4 py-2.5 bg-gradient-to-r from-brand-primary to-brand-accent hover:from-brand-dark hover:to-brand-primary text-white rounded-xl text-xs font-extrabold shadow-glow-red flex items-center justify-center gap-2 transition-all touch-active"
          >
